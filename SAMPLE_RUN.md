...
self-critique behavior can be inspected without an API key.
...
Everything this repository writes is plain ASCII. The captured block below is
not: the model used en dashes and a multiplication sign in its own prose, and a
verbatim capture that had been tidied would not be one. Those characters are
the model's, left as it wrote them.
